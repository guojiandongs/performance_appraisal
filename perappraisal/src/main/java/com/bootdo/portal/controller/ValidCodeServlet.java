  ...
  extends HttpServlet
{
  ...
  { 
	  ...
	  if ( num%3 == 0) {
		  generateNumber(req,resp);
	  //} else  if ( num%3 == 1) {
		 // generateCharacter(req,resp);
	  } else   {  
	  generateQuestion(req,resp);
	  }
...
private void generateNumber(HttpServletRequest req, HttpServletResponse resp)
		    throws ServletException, IOException
  {
	    ...
	    req.setAttribute("valid_code" + vtype, sRand);
	    HttpSession session = req.getSession();
	     session.setAttribute("valid_code" + vtype, sRand);
...
private void drawThickLine(Graphics g, int x1, int y1, int x2, int y2, int thickness, Color c)
  ...
  @SuppressWarnings("unchecked")
public void generateCharacter(HttpServletRequest request, HttpServletResponse response)
             throws ServletException,IOException {
     ...
     //  request.setCharacterEncoding("utf-8");
      ...
      // request.getSession().setAttribute("checkcode",random);
       ...
       request.setAttribute("valid_code" + vtype, random);
       // 将图形写给浏览器
  ...
  private void generateQuestion(HttpServletRequest request, HttpServletResponse response)
          throws ServletException,IOException {
...
private String generateCheckCode(HttpServletRequest request) {
   ...
   // request.getSession().setAttribute("VERIFY_CODE", result); 
    ...
    request.setAttribute("valid_code" + vtype, String.valueOf(result));
    HttpSession session = request.getSession();
    session.setAttribute("valid_code" + vtype, String.valueOf(result));
    return checkCode;  
}   
